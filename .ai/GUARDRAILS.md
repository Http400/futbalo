...
1. **NEVER** install or lock dependencies with `npm` or `yarn` — this repo is pnpm-only (`pnpm-lock.yaml` + committed `.npmrc`); always use `pnpm` / `pnpm --filter @futbalo/<name> <script>`.
2. **NEVER** hand-edit files under `services/auth-service/src/generated/**` — this is Prisma client output; regenerate with `pnpm --filter @futbalo/auth-service db:generate`.
