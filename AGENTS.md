...
pnpm + Turborepo monorepo: React 19 front-ends (Vite), an Express 5 + Prisma 7 + PostgreSQL auth service, and shared `types` / `ui` packages. Node `>=20.16.0`, pnpm `>=10.0.0`, TypeScript 6 strict mode everywhere.
...
- **Check the package latest version** before adding it - training-data memory or Context7 might be stale
